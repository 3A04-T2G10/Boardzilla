export const SET_STICKIES = "SET_STICKIES";
export const ADD_STICKY = "ADD_STICKY";
export const UPDATE_STICKY = "UPDATE_STICKY";
export const REMOVE_STICKY = "REMOVE_STICKY";
export const UPDATE_STICKY_LAYOUT = "UPDATE_STICKY_LAYOUT";

export const SET_NEWS = "SET_NEWS";
export const ADD_NEWS = "ADD_NEWS";
export const UPDATE_NEWS = "UPDATE_NEWS";
export const REMOVE_NEWS = "REMOVE_NEWS";
export const UPDATE_NEWS_LAYOUT = "UPDATE_NEWS_LAYOUT";

export const SET_STOCKS = "SET_STOCKS";
export const ADD_STOCK = "ADD_STOCK";
export const UPDATE_STOCK = "UPDATE_STOCK";
export const REMOVE_STOCK = "REMOVE_STOCK";
export const UPDATE_STOCK_LAYOUT = "UPDATE_STOCK_LAYOUT";
